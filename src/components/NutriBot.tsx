...
export default function NutriBot({
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (validateForm()) {
      ...
      let report = localStorage.getItem("report-analysys");
      let reportSummary = "";
      if (report) {
        report = JSON.parse(report);
        if (report) reportSummary = Object.values(report).join(" ");
      }
      ...
      try {
        ...
        toast.error("Taking too time to generate", {
          description:
            "We Have Initiated the process, Check You Meal Plan after 10 sec",
        });
        console.error(error);
      } finally {
        setLoading(false);
      ...
      <div className="relative mr-40 p-4 rounded-xl border bg-card text-card-foreground shadow-sm">
        <div className="space-y-4">
          ...
          <p className="text-sm text-transparent bg-gradient-to-r from-neutral-700 to-neutral-500 bg-clip-text">
            Get personalized meal plans, nutrition advice, and dietary
            recommendations tailored to your specific needs and health goals.
          </p>
          ...
          <div className="bg-[#ffe3ee] p-4 rounded-lg mb-6">
            ...
            <ul className="space-y-2">
              ...
              <li className="flex items-start gap-2">
                ...
                <span className="text-sm text-gray-700">
                  Gives Exact Nutrition Value for the Meal
                </span>
              </li>
            </ul>
          </div>
        ...
        <form
          className="mt-6 flex flex-col sm:flex-row justify-between pt-4 border-t gap-4"
          ...
          <div className="w-full sm:w-auto">
            <Select required onValueChange={(value) => setDuration(value)}>
              ...
              <SelectContent className="bg-white border text-black">
                <SelectItem
                  value="3"
                  className="hover:bg-[#FFC0D9] hover:text-black"
                >
                  3 Days
                </SelectItem>
                <SelectItem
