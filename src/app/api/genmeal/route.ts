...
export async function POST(request: Request) {
  try {
    ...
    const { userMetadata, planDuration, userId, reportSummary } =
      await request.json();
    ...
    const prompt = `Generate a ${planDuration}-day meal plan for a person with:
      ...
      - Dietary preferences: ${JSON.stringify(
        userMetadata.dietaryPreferences || []
      )}
      - Current Medical report Summary: ${reportSummary}
      ...
      if (genError instanceof Error) {
        if (
          ...
          return NextResponse.json(
            {
              error:
                "Request timed out. Please try again with fewer days or simplified requirements.",
            },
            { status: 504 }
          );
        }
